...
class SimpleVehicleControlCutinFront(SimpleVehicleControlAdapted):

    def run_step(self):
        super(SimpleVehicleControlCutinFront, self).run_step()

        # Hardcoded location trigger for right indicator (lane following cutin hero scenario, Town03)
        if -85.0 < self._actor.get_location().y < -50.0:
            self._set_vehicle_light_state(carla.VehicleLightState.RightBlinker)
